...
from model.hmm.hmm_utils import normalize, log_normalize, \
    iter_from_X_lengths, log_mask_zero
from model.hmm import _hmmc
...
class _BaseHMM(BaseEstimator):
    ...
    def _decode_viterbi_new(self, X):
        framelogprob = self._compute_log_likelihood(X)
        return self._do_viterbi_pass_new(framelogprob)
    ...
    def decode_new(self, X, lengths=None, algorithm=None):
        """Find most likely state sequence corresponding to ``X``.
        ...
        decoder = {
            "viterbi": self._decode_viterbi_new,
            "map": self._decode_map
        }[algorithm]
    ...
    def predict_new(self, X, lengths=None):
        """Find most likely state for each ``X[i]`` based on the historical data ``X[0],X[1],...,X[i]``; different from find most likely state sequence.
        ...
        state_sequence : array, shape (n_samples, )
            Labels for each sample from ``X``.
        """
        _, state_sequence = self.decode_new(X, lengths)
        return state_sequence;
...
#		print (iter);
    ...
    def _do_viterbi_pass_new(self, framelogprob):
        n_samples, n_components = framelogprob.shape
        state_sequence, logprob = _hmmc._viterbi_new(
            n_samples, n_components, log_mask_zero(self.startprob_),
            log_mask_zero(self.transmat_), framelogprob)
        return logprob, state_sequence
