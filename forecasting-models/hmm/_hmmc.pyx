...
def _io_forward(int n_samples, int n_components, int[:] S,
             dtype_t[:] log_startprob,
             dtype_t[:, :, :] log_transmat,
             dtype_t[:, :] framelogprob,
             dtype_t[:, :] fwdlattice):
    ...
    with nogil:
        ...
        for t in range(1, n_samples):
            for j in range(n_components):
                for i in range(n_components):
                    work_buffer[i] = fwdlattice[t - 1, i] + log_transmat[i, j, S[t]]
...
def _io_backward(int n_samples, int n_components, int[:] S,
              dtype_t[:] log_startprob,
              dtype_t[:, :, :] log_transmat,
              dtype_t[:, :] framelogprob,
              dtype_t[:, :] bwdlattice):
    ...
    with nogil:
        ...
        for t in range(n_samples - 2, -1, -1):
            for i in range(n_components):
                for j in range(n_components):
                    work_buffer[j] = (log_transmat[i, j, S[t + 1]]
                                      + framelogprob[t + 1, j]
                                      + bwdlattice[t + 1, j])
                bwdlattice[t, i] = _logsumexp(work_buffer)
...
def _compute_io_log_xi_sum(int n_samples, int n_components, int n_input_components,
			int[:] S,
                        dtype_t[:, :] fwdlattice,
                        dtype_t[:, :, :] log_transmat,
                        dtype_t[:, :] bwdlattice,
    ...
    cdef int t, i, j
    cdef dtype_t[:, :, ::view.contiguous] work_buffer = \
        np.full((n_components, n_components, n_input_components), -INFINITY)
    cdef dtype_t logprob = _logsumexp(fwdlattice[n_samples - 1])
    ...
    with nogil:
        for t in range(n_samples - 1):
            for i in range(n_components):
                for j in range(n_components):
                    work_buffer[i, j, S[t+1]] = (fwdlattice[t, i]
                                         + log_transmat[i, j, S[t+1]]
                                         + framelogprob[t + 1, j]
                                         + bwdlattice[t + 1, j]
                                         - logprob)
            ...
            for i in range(n_components):
                for j in range(n_components):
                    for k in range(n_input_components):	                
                        log_xi_sum[i, j, k] = _logaddexp(log_xi_sum[i, j, k], work_buffer[i, j, k])
...
def _viterbi_new(int n_samples, int n_components,
             dtype_t[:] log_startprob,
             dtype_t[:, :] log_transmat,
             dtype_t[:, :] framelogprob):
    ...
    with nogil:
        ...
        for t in range(0, n_samples):
                state_sequence[t] = where_from = \
                            _argmax(viterbi_lattice[t])
        logprob = viterbi_lattice[n_samples - 1, where_from]

    return np.asarray(state_sequence), logprob

def _io_viterbi(int n_samples, int n_components, int[:] S,
             dtype_t[:] log_startprob,
             dtype_t[:, :, :] log_transmat,
             dtype_t[:, :] framelogprob):
    ...
    with nogil:
        ...
        for t in range(1, n_samples):
            for i in range(n_components):
                for j in range(n_components):
                    work_buffer[j] = (log_transmat[j, i, S[t]]
                                      + viterbi_lattice[t - 1, j])
        ...
        for t in range(n_samples - 2, -1, -1):
            for i in range(n_components):
                work_buffer[i] = (viterbi_lattice[t, i]
                                  + log_transmat[i, where_from, S[t+1]])

            state_sequence[t] = where_from = _argmax(work_buffer)

    return np.asarray(state_sequence), logprob

def _io_viterbi_new(int n_samples, int n_components, int[:] S,
             dtype_t[:] log_startprob,
             dtype_t[:, :, :] log_transmat,
             dtype_t[:, :] framelogprob):
